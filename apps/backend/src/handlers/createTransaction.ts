...
export const handler: APIGatewayProxyHandlerV2WithJWTAuthorizer = async (event) => {
  try {
    ...
    // Validate the body before the getUserPlan() DynamoDB round trip, so a malformed request
    ...
    const transaction = await createTransaction(repository, userId, input, plan);
