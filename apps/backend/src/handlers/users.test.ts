...
import { handler as withdrawUserHandler } from './withdrawUser';
...
describe('withdrawUser handler', () => {
  it('returns 401 when the JWT sub claim is missing', async () => {
    const event = buildEvent();

    const result = await withdrawUserHandler(event, {} as never, () => undefined);
